...
##############################################################
#  script: batch_recombination_profiling_by_parent_genomes.pl
...
#  example: perl batch_recombination_profiler_by_parent_genomes.pl -s Master_Sample_Table.txt -n 3 -l 100 -d 5000 -q 30 -b batch_id
##############################################################
			...
			if (exists $blocks_by_segregation_pattern{$chr}{$segregation_pattern}) {
				...
				foreach my $block_id (@block_subclass) {
					...
					#     ;
					    ...
					    # the GC is not in adjacency to COs (or overlapped with Type6_GC, Type17_GC)
					    $recombination_event_id++;
					    my $event_type = "GC";
