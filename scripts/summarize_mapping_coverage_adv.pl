...
print $output_fh "sample\tref_genome\ttotal_reads\tmapping_rate\tpairing_rate\t(>=$min_depth_cutoff)_depth_proportion\tzero_depth_proportion\tmean_depth\tmedian_depth\tstdev_depth";

foreach my $chr (@refseq) {
    print $output_fh "\t${chr}_mean_depth\t${chr}_median_depth\t${chr}_stdev_depth";
}
print $output_fh "\n";
...
print $output_fh "\t$depth_stat{'mean_depth'}\t$depth_stat{'median_depth'}\t$depth_stat{'stdev_depth'}";
foreach my $chr (@refseq) {
    print $output_fh "\t$depth_stat{'chr_mean_depth'}{$chr}\t$depth_stat{'chr_median_depth'}{$chr}\t$depth_stat{'chr_stdev_depth'}{$chr}";
}
print $output_fh "\n";
...
sub parse_depth_file {
    ...
    my $stdev_depth = $total_depth_stat->standard_deviation();
    $stat{"mean_depth"} = sprintf("%.2f", $mean_depth);
    $stat{"median_depth"} = sprintf("%.2f", $median_depth);
    $stat{"stdev_depth"} = sprintf("%.2f", $stdev_depth);
	...
	my $chr_stdev_depth = $chr_depth_stat->standard_deviation();
	$stat{"chr_mean_depth"}{$chr} = sprintf("%.2f", $chr_mean_depth);
	$stat{"chr_median_depth"}{$chr} = sprintf("%.2f", $chr_median_depth);
	$stat{"chr_stdev_depth"}{$chr} = sprintf("%.2f", $chr_stdev_depth);
