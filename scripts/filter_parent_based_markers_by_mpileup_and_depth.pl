...
#  example: perl filter_parent_based_markers_by_mpileup_and_depth.pl -depth_detail mpileup.gz -depth_summary depth.summary.txt -i markers.vcf.gz -o filtered.markers.vcf.gz -filter_by_depth_variation yes
##############################################################

my ($input, $output, $filter_by_depth_variation, $depth_summary, $mpileup);
...
if ($filter_by_depth_variation eq "yes") {
    $depth_summary_fh = read_file($depth_summary);
    %depth_summary = parse_depth_summary_file($depth_summary_fh);
}
