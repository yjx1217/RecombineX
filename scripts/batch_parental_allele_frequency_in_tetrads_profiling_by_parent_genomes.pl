...
##############################################################
#  script: batch_parental_allele_frequency_in_tetrads_profiling_by_parent_genomes.pl 
#  author: Jia-Xing Yue (GitHub ID: yjx1217)
#  last edited: 2022.07.24
#  description: calculate parental allele frequency in tetrads
#  example: perl batch_parental_allele_frequency_in_tetrads_profiling_by_parent_genomes.pl -s Master_Sample_Table.txt -batch_id batch_id -q 30 -m raw -prefix prefix 
##############################################################
...
# count to frequency
my $output_by_genome1 = "$prefix.$cross_pair.$genome1_tag.parental_allele_frequency.$mode.txt";
my $output_by_genome2 = "$prefix.$cross_pair.$genome2_tag.parental_allele_frequency.$mode.txt";

my $output_by_genome1_fh = write_file($output_by_genome1);
print $output_by_genome1_fh "cross_pair\tref\tchr\tpos\tgenotype\ttotal_allele_count\tgenotype_match_allele_count\tfreq\n";

my $output_by_genome2_fh = write_file($output_by_genome2);
print $output_by_genome2_fh "cross_pair\tref\tchr\tpos\tgenotype\ttotal_allele_count\tgenotype_match_allele_count\tfreq\n";
...
foreach my $reftag (sort keys %marker_allele_freq) {
	...
	foreach my $pos (sort {$a <=> $b} keys %{$marker_allele_freq{$reftag}{$chr}}) {
		...
		if ($total_allele_count > 0) {
		    ...
		    if ($ignore_na eq "yes") {
			if ($genotype ne "NA") {
			    if ($reftag eq $genome1_tag) {
				print $output_by_genome1_fh "$cross_pair\t$reftag\t$chr\t$pos\t$genotype\t$total_allele_count\t$marker_allele_freq{$reftag}{$chr}{$pos}{$genotype}{'count'}\t$marker_allele_freq{$reftag}{$chr}{$pos}{$genotype}{'freq'}\n";
			    } else {
				print $output_by_genome2_fh "$cross_pair\t$reftag\t$chr\t$pos\t$genotype\t$total_allele_count\t$marker_allele_freq{$reftag}{$chr}{$pos}{$genotype}{'count'}\t$marker_allele_freq{$reftag}{$chr}{$pos}{$genotype}{'freq'}\n";
			    }
			}
		    } else {
			if ($reftag eq $genome1_tag) {
