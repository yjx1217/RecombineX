...
min_block_size=10000 # The minimal marker-bounded block size (bp) to be considered for trustful linkage blocks. Default = "10000" (i.e. 10000 bp).
merging_range=5000 # The distance range (bp) for merging nearby COs. Default = "5000" (i.e. no merging). 
...
output_dir="$batch_id" # The output directory to be created within this current directory. Default = "$batch_id".
...
perl $RECOMBINEX_HOME/scripts/batch_gamete_based_recombination_profiling_by_reference_genome.pl -s $master_sample_table -n $min_marker_number -l $min_block_size \
